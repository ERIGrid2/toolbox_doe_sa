...
START = '2014-03-22 08:00:00'
END = 24 * 3600          # simulate for five hours (12:24 minutes simulation time)
# END = 3600          # simulate for one hour (3:14 minutes)
# END = 24 * 3600          # simulate for one day

# Component parameters:
U_GRID = 'data/grid_uncertainty.json' # grid simulator uncertainty overview
GRID_FILE = 'data/demo_lv_grid.json' # grid topology
GRID_CDM = 'data/grid_cdm_modified.json' # child dependency map of simulated grid
...
def main():
    world = mosaik.World(sim_config, mosaik_config=mosaik_config, debug=True)
    create_scenario(world)
    world.run(until=END)
    # util.plot_execution_time(world, folder='figures\\util')
    # util.plot_execution_time_per_simulator(world, folder='figures\\util')
    # util.plot_execution_graph(world, folder='figures\\util')
    # util.plot_execution_graph_new(world, folder='figures\\util')
    # util.plot_df_graph(world, folder='figures\\util')
...
def create_scenario(world):
    ...
    houses = hhsim.House.create(num=5)
    ...
    for i in range(len(node_ins)):
        world.connect(houses[i], node_ins[i], 'P')
        world.connect(pv_outs[i], node_ins[i], 'P')

    # Create database::
    db = world.start('DB', step_size=60, duration=END)
    hdf5 = db.Database(filename='results\\uq_example_scenario.hdf5')
